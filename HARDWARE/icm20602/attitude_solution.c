...
/* 如果需要用到2个陀螺仪 */
#if 0
void IMU_getValues2(float *values) 
{
    static double lastaccel[3]= {0,0,0};
    int i;
    values[0] = ((float)icm_acc_x) * new_weight + lastaccel[0] * old_weight;
...
/**
  ...
  * return None
  */
quaterInfo_t Q_info2 = {1, 0, 0};  // 全局四元数
float I_ex2, I_ey2, I_ez2;  // 误差积分
static void IMU_AHRSupdate_noMagnetic2(float gx, float gy, float gz, float ax, float ay, float az)
{
	...
	I_ex2 += delta_T * ex;   // integral error scaled by Ki
	I_ey2 += delta_T * ey;
	I_ez2 += delta_T * ez;

	gx = gx+ param_Kp*ex + param_Ki*I_ex2;
	gy = gy+ param_Kp*ey + param_Ki*I_ey2;
	gz = gz+ param_Kp*ez + param_Ki*I_ez2;
